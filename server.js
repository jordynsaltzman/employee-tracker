...
function startTracker() {
    inquirer.prompt(questions).then(function (res, err) {
        if (err) throw err;
...
function viewAllEmployees() {
    connection.query("SELECT first_name as 'First Name' , last_name as 'Last Name', title as 'Title', salary as 'Salary' FROM employee LEFT JOIN role ON employee.role_id = role.id;",
        function (err, res) {
            if (err) throw err;
            console.table(res)
            return startTracker();
...
function addEmployee() {
    connection.query("SELECT * FROM employee", function (err, managerResult) {
        ...
        if (managerResult.length === 0) {
            console.log("You need to enter an employee first.");
            return startTracker();
        };

        let managersList = managerResult.map(function (manager) {
...
function addDepartments() {
    ...
    inquirer.prompt(departmentQuestions)
        .then(function (res) {
            connection.query("INSERT INTO department SET ?", { name: res.department_name }, function (err, res) {
                if (err) throw err;
                viewDepartments();
                startTracker();
            });
...
function addRoles() {
    connection.query("SELECT * FROM department", function (err, departmentResult) {
        ...
        inquirer.prompt(rolesQuestions)
            .then(function (roleQuestionsResult) {
